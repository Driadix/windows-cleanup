...
Вопрос про диски — **одиночный выбор (radio)**, варианты «только X», «только Y», «оба» БЕЗ пересечений:
мультивыбор здесь даёт противоречивые пары («только C:» + «оба») — реальный кейс прогона 0.2.2.
