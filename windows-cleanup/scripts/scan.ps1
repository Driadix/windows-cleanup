...
# -LiteralPath обязателен (кириллица/пробелы); reparse points пропускаются (защита от циклов);
# отчёты пишутся построчно без усечения длинных путей.
...
$rows | Sort-Object GB -Descending | Select-Object -First $Top |
    ForEach-Object { "$($_.GB)`t$($_.Path)" } | Set-Content -Path $dirTop -Encoding UTF8

# --- Топ файлов ---
Get-ChildItem -LiteralPath $Root -Recurse -Force -File |
    Where-Object { -not ($_.Attributes -band [IO.FileAttributes]::ReparsePoint) } |
    Sort-Object Length -Descending | Select-Object -First $Top |
    ForEach-Object { "$($_.Length)`t$($_.LastWriteTime.ToString('yyyy-MM-dd'))`t$($_.FullName)" } |
    Set-Content -Path $fileTop -Encoding UTF8
...
Get-ChildItem -LiteralPath $Root -Recurse -Force -File |
    ...
    ForEach-Object { "$($_.Length)`t$($_.FullName)" } | Set-Content -Path $dupes -Encoding UTF8
